...
import { getCreateStudioAction, getCreateStudioPrimaryControl, getCreateStudioStepStates, type CreateStudioStepStage } from '@/lib/create-studio-action';
...
export default function CreateStudio({ initialCredits, plan }: { initialCredits: number; plan: string }) {
  ...
  function selectFile(selected?: File) {
    ...
    const allowedExtension = ['mp4', 'mov', 'webm', 'mkv'].includes(extension);
    if (!selected.type.startsWith('video/') && !allowedExtension) return setError('Pilih file video MP4, MOV, WebM, atau MKV.');
        ...
        <div className="grid gap-5 py-8 lg:grid-cols-[1.35fr_.65fr]">
          <section className="rounded-[2rem] border border-white/10 bg-white/[0.025] p-4 sm:p-5">
            ...
            {sourceMode === 'upload' ? (!file ? (
              <label className="relative flex min-h-[420px] cursor-pointer flex-col items-center justify-center rounded-[1.5rem] border-2 border-dashed border-white/10 bg-black/10 p-8 text-center">
                <input type="file" accept={VIDEO_ACCEPT} className="absolute inset-0 h-full w-full cursor-pointer opacity-0" onChange={onFileChange} />
                <span className="pointer-events-none w-full max-w-md rounded-2xl border border-cyan-300/30 bg-cyan-300/[0.08] px-6 py-7"><span className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-cyan-300 text-2xl text-slate-950">▶</span><span className="mt-5 block text-lg font-black">Pilih Video dari HP</span><span className="mt-2 block text-sm text-slate-500">MP4, MOV, WebM, MKV • maksimal 500 MB</span></span>
              </label>
            ) : (
              <><video src={url} controls onLoadedMetadata={metadata} className="aspect-video w-full rounded-2xl bg-black object-contain"/><div className="mt-4 flex items-center justify-between"><div><div className="text-sm font-bold">{file.name}</div><div className="text-xs text-slate-600">{formatSize(file.size)} • {formatTime(duration)}</div></div><label className="relative cursor-pointer rounded-xl border border-cyan-300/20 px-4 py-3 text-xs font-bold text-cyan-300"><input type="file" accept={VIDEO_ACCEPT} className="absolute inset-0 h-full w-full cursor-pointer opacity-0" onChange={onFileChange} />Ganti video</label></div></>
            )) : (
          ...
          <aside className="rounded-[2rem] border border-white/10 bg-white/[0.025] p-5 sm:p-6">
            ...
            {sourceMode === 'youtube' ? (
              ...
              <button type="button" disabled={primaryDisabled} onClick={() => void prepare()} className="mt-6 w-full rounded-xl bg-cyan-300 px-4 py-3.5 text-sm font-black text-slate-950 disabled:cursor-not-allowed disabled:opacity-40">{busy ? '⏳ Proses 01–04 sedang berjalan…' : !isOwner && credits < batchCount ? 'Kredit tidak cukup' : '🚀 Mulai Proses AI →'}</button>
            )}
...
function formatTime(seconds: number) { if (!seconds) return 'memuat durasi…'; const m = Math.floor(seconds / 60); const s = Math.floor(seconds % 60); return `${m}:${String(s).padStart(2, '0')}`; }

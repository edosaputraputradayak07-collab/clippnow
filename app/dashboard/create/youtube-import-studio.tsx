...
type Clip = { videoId: number | string; videoUrl: string; videoMsDuration?: number; title?: string; transcript?: string; viralScore?: string; viralReason?: string; clipEditorUrl?: string };
...
export default function YouTubeImportStudio({ plan }: Props) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const response = await fetch(`/api/youtube/vizard?project_id=${encodeURIComponent(projectId)}`, { cache: 'no-store' });
        const data = await response.json().catch(() => ({}));
        if (cancelled) return;
        if (!response.ok) throw new Error(data.error ?? 'Gagal mengambil hasil AI.');
        if (data.status === 'complete') {
          setClips(Array.isArray(data.videos) ? data.videos : []);
          setBusy(false);
          setStatus(`${Array.isArray(data.videos) ? data.videos.length : 0} clip selesai dibuat.`);
          return;
        }
        setStatus('AI sedang menganalisis video YouTube…');
  ...
  async function start() {
    ...
    setBusy(true); setError(''); setClips([]); setProjectId(''); setStatus('Mengirim video ke AI engine…');
    try {
      const response = await fetch('/api/youtube/vizard', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ youtube_url: link.trim(), count, language: 'id', project_name: 'Vidklipral YouTube' }),
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error ?? 'Gagal memulai proses YouTube.');
      setProjectId(String(data.provider_project_id));
      setStatus('Video diterima. AI mulai mencari momen viral…');
    } catch (cause) {
    ...
    <section className="mb-6 overflow-hidden rounded-[2rem] border border-cyan-300/20 bg-cyan-300/[0.035] p-5 sm:p-6">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="max-w-xl">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-500">Tidak perlu download lalu upload manual. Vidklipral mengirim URL ke engine pemrosesan resmi yang mendukung sumber YouTube, lalu mengambil hasil clip kembali ke dashboard.</p>
        </div>
        <div className="w-full max-w-xl rounded-2xl border border-white/10 bg-black/20 p-4">
          ...
          <div className="mt-3 flex items-center gap-2">{[5, 8, 10].map(value => <button key={value} type="button" onClick={() => setCount(value)} className={`rounded-lg border px-3 py-2 text-[11px] font-black ${count === value ? 'border-cyan-300/50 bg-cyan-300/10 text-cyan-200' : 'border-white/10 text-slate-500'}`}>{value} clip</button>)}</div>
      ...
      {clips.length > 0 && <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{clips.map((clip, index) => <article key={String(clip.videoId)} className="overflow-hidden rounded-2xl border border-white/10 bg-black/30"><video src={clip.videoUrl} controls preload="metadata" className="aspect-[9/16] w-full bg-black object-contain"/><div className="p-4"><div className="text-sm font-black">{clip.title || `Clip ${index + 1}`}</div><div className="mt-1 text-[11px] text-cyan-300">Viral score: {clip.viralScore ?? '—'}</div>{clip.viralReason && <p className="mt-2 text-xs leading-5 text-slate-500">{clip.viralReason}</p>}<div className="mt-3 flex gap-2"><a href={clip.videoUrl} target="_blank" rel="noreferrer" className="rounded-lg bg-cyan-300 px-3 py-2 text-[11px] font-black text-slate-950">Buka / Simpan</a>{clip.clipEditorUrl && <a href={clip.clipEditorUrl} target="_blank" rel="noreferrer" className="rounded-lg border border-white/10 px-3 py-2 text-[11px] font-black text-slate-300">Edit</a>}</div></div></article>)}</div>}
      ...
      <div className="mt-1 text-[10px] text-slate-700">Mode: {plan === 'owner' ? 'Owner' : 'User'} • AI YouTube provider</div>
    </section>
  );
}

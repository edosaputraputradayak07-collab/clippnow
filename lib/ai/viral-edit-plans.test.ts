...
describe('buildViralEditPlans', () => {
  ...
  it('returns multiple ranked clips up to the requested count', () => {
    const plans = buildViralEditPlans({ durationSeconds: 180, format: '9:16', goal: 'tiktok', transcript, count: 7 });
  ...
  it('never returns more than ten clips and avoids overlapping ranges', () => {
    const plans = buildViralEditPlans({ durationSeconds: 180, format: '9:16', goal: 'tiktok', transcript, count: 20 });

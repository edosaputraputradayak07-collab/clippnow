...
export default function ForgotPasswordPage() {
  ...
  async function sendCode(event: FormEvent<HTMLFormElement>) {
    ...
    setStep('otp');
    setMessage('Kode reset 6 digit sudah dikirim ke email kamu.');
  }
  ...
  async function verifyCode(event: FormEvent<HTMLFormElement>) {
    ...
    setLoading(false);
    if (verifyError) { setError('Kode salah atau sudah kedaluwarsa. Minta kode baru.'); return; }
    setStep('password');
    setMessage('Kode benar. Sekarang buat password baru.');
  }
  ...
  return <main className="flex min-h-screen items-center justify-center bg-[#05070d] px-5 py-8 text-white sm:px-8">
    <div className="w-full max-w-md rounded-[2rem] border border-white/10 bg-white/[.035] p-7 shadow-2xl sm:p-9">
      <a href="/" className="text-xl font-black">Vid<span className="text-cyan-300">klipral</span></a>
      ...
      <p className="mt-3 text-sm leading-6 text-slate-500">Pulihkan akun dengan kode reset yang dikirim ke email kamu.</p>
      {step === 'email' && <form onSubmit={sendCode} className="mt-8 space-y-4"><label className="block"><span className="mb-2 block text-xs font-bold text-slate-400">Email akun</span><input required type="email" autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm outline-none focus:border-cyan-300/60" /></label><button disabled={loading} className="w-full rounded-xl bg-cyan-300 px-4 py-3 text-sm font-black text-slate-950 disabled:opacity-50">{loading ? 'Mengirim kode…' : 'Kirim kode →'}</button></form>}
      {step === 'otp' && <form onSubmit={verifyCode} className="mt-8 space-y-4"><div className="rounded-xl border border-cyan-300/10 bg-cyan-300/5 px-4 py-3 text-xs text-slate-400">Kode dikirim ke <b className="text-white">{email}</b></div><label className="block"><span className="mb-2 block text-xs font-bold text-slate-400">Kode 6 digit</span><input required inputMode="numeric" pattern="[0-9]{6}" maxLength={6} value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, '').slice(0, 6))} className="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-center text-2xl font-black tracking-[.5em] outline-none focus:border-cyan-300/60" /></label><button disabled={loading || otp.length !== 6} className="w-full rounded-xl bg-cyan-300 px-4 py-3 text-sm font-black text-slate-950 disabled:opacity-50">{loading ? 'Memeriksa…' : 'Verifikasi kode →'}</button><button type="button" onClick={() => { setStep('email'); setOtp(''); setMessage(''); setError(''); }} className="w-full py-2 text-xs font-bold text-slate-500 hover:text-white">Kirim ulang / ganti email</button></form>}

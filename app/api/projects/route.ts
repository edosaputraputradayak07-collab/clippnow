...
export async function POST(request: Request) {
  ...
  if (!sourcePath) {
    return NextResponse.json({ error: 'Source video tidak valid.' }, { status: 400, headers: noStoreHeaders() });
  }
  ...
  if (projectError || !project) {
    return NextResponse.json({ error: 'Gagal membuat project.' }, { status: 500, headers: noStoreHeaders() });
  }

  const { data: balance, error: creditError } = await admin.rpc('reserve_clippnow_credit', {
    p_user_id: user.id,
    p_reference: project.id,
  });

  if (creditError) {
    await supabase.from('projects').delete().eq('id', project.id);
    const message = creditError.message.includes('insufficient_credits')
      ? 'Kredit kamu habis. Beli paket untuk melanjutkan.'
      : 'Gagal menggunakan kredit.';
    return NextResponse.json({ error: message }, {
      status: creditError.message.includes('insufficient_credits') ? 402 : 500,

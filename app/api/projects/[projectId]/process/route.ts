...
export async function POST(_request: Request, context: RouteContext) {
  ...
  const { error: projectUpdateError } = await supabase.from('projects').update({ status: 'processing' }).eq('id', project.id).eq('user_id', user.id);
  ...
  await supabase.from('usage_events').insert({ user_id: user.id, project_id: project.id, job_id: job.id, event_type: 'job_queued', credits, metadata: { mode: project.mode, durationMs } });

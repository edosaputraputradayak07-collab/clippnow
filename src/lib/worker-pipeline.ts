...
export async function runContentEngine(
  steps: ContentEngineSteps,
  input: { projectId: string; mode: ContentMode },
) {
  if (!input.projectId.trim()) throw new Error('PROJECT_ID_REQUIRED');

  const source = await steps.loadSource();
  if (!source.path || !Number.isFinite(source.durationMs) || source.durationMs <= 0) {
  ...
  if (outputs.length < MIN_OUTPUTS || outputs.length > MAX_OUTPUTS) {

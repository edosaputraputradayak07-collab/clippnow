      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-5 py-8 lg:grid-cols-[1.35fr_.65fr]">
          <section className="rounded-[2rem] border border-white/10 bg-white/[0.025] p-4 sm:p-5">
            ...
            {sourceMode === 'upload' ? (!file ? <label className="relative flex min-h-[420px] cursor-pointer flex-col items-center justify-center overflow-hidden rounded-[1.5rem] border-2 border-dashed border-white/10 bg-black/10 p-8 text-center hover:border-cyan-300/40 active:border-cyan-300/60"><input type="file" accept="video/*" className="absolute inset-0 z-10 h-full w-full cursor-pointer opacity-0" onChange={onFileChange} /><div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-cyan-300 text-2xl text-slate-950">▶</div><h2 className="mt-6 text-lg font-black">Klik untuk memilih video</h2><p className="mt-2 text-sm text-slate-600">MP4, MOV, WebM, MKV • maksimal 500 MB</p></label> : <><video src={url} controls onLoadedMetadata={metadata} className="aspect-video w-full rounded-2xl bg-black object-contain"/><div className="mt-4 flex items-center justify-between"><div><div className="text-sm font-bold">{file.name}</div><div className="text-xs text-slate-600">{formatSize(file.size)} • {formatTime(duration)}</div></div><label className="relative cursor-pointer text-xs font-bold text-cyan-300"><input type="file" accept="video/*" className={pickerInput} onChange={onFileChange} />Ganti video</label></div></>) : (

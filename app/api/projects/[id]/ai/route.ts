...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    for (let index = 1; index < plans.length; index += 1) {
      const plan = plans[index]; const reference = crypto.randomUUID();
      if (!owner) { const { error: creditError } = await admin.rpc('reserve_clippnow_credit', { p_user_id: user.id, p_reference: reference }); if (creditError) { for (const ref of extraReferences) await admin.rpc('release_clippnow_credit', { p_user_id: user.id, p_reference: ref }); await admin.from('projects').delete().in('id', createdExtraIds).eq('user_id', user.id); return NextResponse.json({ error: 'Kredit tidak cukup untuk menyelesaikan batch. Tidak ada kredit tambahan yang dipakai.' }, { status: 402, headers: noStoreHeaders() }); } extraReferences.push(reference); }
      const start = offset + plan.clip.startSeconds; const finish = offset + plan.clip.endSeconds; const words = selectClipWords(transcription.words ?? [], start, finish); const childEditPlan = { ...plan, transcript: localSegments, transcript_origin_seconds: offset, words, viral_explainer: explainerPlan };
      ...
      if (jobError) { await admin.from('projects').delete().eq('id', child.id).eq('user_id', user.id); if (!owner) await admin.rpc('release_clippnow_credit', { p_user_id: user.id, p_reference: reference }); for (const ref of extraReferences) if (ref !== reference) await admin.rpc('release_clippnow_credit', { p_user_id: user.id, p_reference: ref }); await admin.from('projects').delete().in('id', createdExtraIds).eq('user_id', user.id); return NextResponse.json({ error: 'Gagal membuat render job batch.' }, { status: 500, headers: noStoreHeaders() }); }

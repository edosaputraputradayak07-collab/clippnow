...
export async function GET(request: Request) {
  const secret = process.env.CRON_SECRET?.trim() || process.env.WORKER_RUNNER_SECRET?.trim();
  if (!secret) return NextResponse.json({ error: 'WORKER_CRON_NOT_CONFIGURED' }, { status: 503 });
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'WORKER_CRON_FAILED';

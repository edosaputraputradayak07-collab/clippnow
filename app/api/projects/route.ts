...
import { vidklipralRenderWorkflow } from '@/workflows/vidklipral-render';
import { buildVideoWorkflowInput } from '@/lib/vidklipral/pipeline/workflow-request';
...
export async function POST(request: Request) {
  ...
  const { format, start_seconds: startSeconds, end_seconds: endSeconds } = validation.value;
  const admin = createAdminClient();
  const { data: profile } = await admin.from('profiles').select('plan').eq('id', user.id).single();
  const owner = profile?.plan === 'owner';
  const creditReference = crypto.randomUUID();
  const { data: project, error: projectError } = await supabase.from('projects').insert({ user_id: user.id, name, original_filename: typeof body.original_filename === 'string' ? body.original_filename.slice(0, 255) : null, start_seconds: startSeconds, end_seconds: endSeconds, format, source_path: sourcePath, status: 'queued', credit_reference: creditReference }).select('id,credit_reference').single();
  ...
  try {
    const run = await start(vidklipralRenderWorkflow, [buildVideoWorkflowInput({
    ...
    return NextResponse.json({
      ...
      job_id: job.id,
      run_id: run.runId,
      credits_remaining: owner ? null : balance,
    ...
    if (!owner) await admin.rpc('release_clippnow_credit', { p_user_id: user.id, p_reference: creditReference });
    return NextResponse.json({ error: 'Gagal menjalankan render workflow.' }, { status: 500, headers: noStoreHeaders() });
  }
}

...
export async function renderClip(input:FfmpegInput,onProgress?:(p:RenderProgress)=>void):Promise<{outputPath:string}> {
  ...
  return new Promise((resolve,reject)=>{
    ...
    child.stdout.on('data',(chunk:string)=>{
      ...
      let outTime=0; let percent=0;
      for(const line of lines){ const [key,val]=line.split('='); if(key==='out_time_us') outTime=Number(val)/1_000_000; else if(key==='progress' && val==='end') percent=100; }
      if(outTime>0) percent=Math.min(99,Math.round((outTime/input.durationSeconds)*100));
      onProgress?.({percent,outTimeSeconds:outTime});
    });
    child.stderr.on('data',(chunk:string)=>{ stderr=(stderr+chunk).slice(-4000); });
    child.on('error',reject);
    child.on('close',(code)=>{ if(code===0) resolve({outputPath:input.outputPath}); else reject(new Error(`FFmpeg failed (${code}): ${stderr.replace(/\s+/g,' ').trim()}`)); });
  });
}
